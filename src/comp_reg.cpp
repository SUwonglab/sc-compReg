...
Rcpp::List compReg(const arma::sp_mat& O1,
    ...
    try {
        ...
        arma::uvec f2 = arma::uvec(peakNameIntersect2.size(), arma::fill::zeros);
        arma::uvec f1 = arma::uvec(peakNameIntersect1.size(), arma::fill::zeros);
        ind = 0;
        ...
        arma::uvec temp = arma::find(d1 == 0);
        d1Zero = temp.n_elems;
        temp = arma::find(d2 == 0);
        d2Zero = temp.n_elems;
        unsigned int m = peakNameIntersect1.size(); // length(peakNameIntersect1) = length(peakNameIntersect2)
        unsigned int OMeanNRows = std::max({peakNameIntersect1.size(), d1Zero, d2Zero}); // TODO: check with Duren here
        ...
        f1Idx;
        double loopElem;
        unsigned int nCols = O1Mean.n_cols;
        arma::uvec mIdx = arma:::regspace<arma::uvec>(0, m - 1);
        for (int i = 0; i < K1; ++i) {
            ...
            for (arma::uvec::iterator it = f1.begin(); it != f1.end(); ++it) {
                ...
                O1Mean.elem(mIdx * nCols + i) = avg / idx.n_elem;
                ++f1Idx;
            }
        }
        ...
        return Rcpp::List::create(Named("0") = 0);
    } catch(...) {
